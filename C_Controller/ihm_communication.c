...
long  get_setting_Tplat_ms  ()
{
    ...
    const float Ti_ms = get_setting_VT_mL()*1000 / get_setting_Vmax_Lpm() / 60000;
...
bool send_DATA(float P_cmH2O, float VolM_Lpm, float Vol_mL, float Pplat_cmH2O, float PEP_cmH2O)
{
    ...
    return snprintf(frame, MAX_FRAME, "DATA msec_:%06d Vol__:%04d Deb__:%c%03d Paw__:%c%03d" CS8,
                    (int)(get_time_ms() % 1000000l), (int)Vol_mL, sign(VolM_Lpm), (int)VolM_Lpm, sign(P_cmH2O), abs((int)P_cmH2O)) > 0
           && send(frame);
}
    ...
    while (true) {
        ...
        *(pcs8+strlen(CS8))='\0';
        ...
        if ((pl = payload(frame, INIT)) || !initSent) {
            ...
            process(&pf, TPLAT, TPLAT_FMT, &ignored_Tplat_ms  ) ||
